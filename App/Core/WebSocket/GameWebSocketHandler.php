...
class GameWebSocketHandler
{
        ...
        try {
            switch ($data['type']) {
                ...
                case 'admin_sticker_add':
                    $this->handleAdminStickerAdd($server, $fd, $data);
                    break;
                case 'admin_sticker_delete':
                    $this->handleAdminStickerDelete($server, $fd, $data);
                    break;
                case 'admin_sticker_list':
                    $this->handleAdminStickerList($server, $fd, $data);
                    break;
                case 'admin_get_upload_config':
                    $this->handleAdminGetUploadConfig($server, $fd, $data);
                    break;
                case 'get_stickers':
     ...
     * 管理员添加自定义表情
     */
    private function handleAdminStickerAdd(Server $server, int $fd, array $data): void
    {
        if (!$this->verifyAdmin($fd, $data, $server)) return;
        ...
        if (empty($url) || !preg_match('#^https?://.+#i', $url) || mb_strlen($url) > 500) {
            $this->sendError($server, $fd, '请输入有效的图片 URL');
            return;
        }

        // StickerService: Redis 写入 → 标记 sync → 异步 SQLite
        $sticker = StickerService::add($name, $url);

        $this->sendToPlayer($server, $fd, [
            'type' => 'admin_sticker_added',
            'sticker' => $sticker,
        ]);
     ...
     * 管理员删除自定义表情
     */
    private function handleAdminStickerDelete(Server $server, int $fd, array $data): void
    {
        if (!$this->verifyAdmin($fd, $data, $server)) return;

        $id = Sanitizer::identifier($data['id'] ?? '');
        if (empty($id)) {
            $this->sendError($server, $fd, '表情 ID 不能为空');
            return;
        }

        // StickerService: Redis 删除 → 标记 delete sync → 异步 SQLite 删除
        StickerService::delete($id);

        $this->sendToPlayer($server, $fd, [
            'type' => 'admin_sticker_deleted',
            'id' => $id,
        ]);
     ...
     * 管理员获取自定义表情列表
     */
    private function handleAdminStickerList(Server $server, int $fd, array $data): void
    {
        if (!$this->verifyAdmin($fd, $data, $server)) return;

        // StickerService: Redis 优先 → 回退 SQLite
        $stickers = StickerService::list();

        $this->sendToPlayer($server, $fd, [
            'type' => 'admin_stickers_list',
            'stickers' => $stickers,
        ]);
    }

    /**
     * 管理员获取图床上传配置（动态配置，不写死在前端）
     */
    private function handleAdminGetUploadConfig(Server $server, int $fd, array $data): void
    {
        if (!$this->verifyAdmin($fd, $data, $server)) return;
